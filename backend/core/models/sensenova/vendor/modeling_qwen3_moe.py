# SushiUI vendored copy — UNMODIFIED (reuses the repointed `Qwen3Attention` from
# `modeling_qwen3.py`, so its attention already runs through SushiUI's conduit).
#
# Vendored only because `modeling_neo_chat.py` imports `Qwen3MoeForCausalLM`
# unconditionally; the SenseNova-U1.5-8B-MoT checkpoint this loader targets is
# the dense (non-MoE) backbone (`NEOLLMConfig` / `Qwen3ForCausalLM`), so this
# file's classes are never instantiated by that checkpoint but are required
# for the package to import.
#
# Source: https://github.com/OpenSenseNova/SenseNova-U1, branch `feat/u1.5`
#         (commit a1ce053d25835e0785a0869ca1c97e717212ef64), file
#         `src/sensenova_u1/models/neo_unify/modeling_qwen3_moe.py`, retrieved 2026-08-21.
