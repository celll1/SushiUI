...
| Block swap boundary | Training: `SenseNovaArchHandler.setup_block_swap` → `sensenova_ops.setup_block_swap` → `BranchedLayerOffloadConductor`; keys are `(understanding|generation, physical_layer_index)` | 0..41 of 42 layers; gradient checkpointing and ring size >=2 required. LoRA streams frozen base only; full FT writes dirty bundles back. Generation still has no block-swap driver |
