...
interface QuantizedExportSectionProps {
  /** Loaded model architecture; used only to re-read status on a model change. */
  arch?: string | null;
...
export default function QuantizedExportSection({
  ...
  if (!status || (!status.exportable && !hasJob)) return null;
    ...
    <Card
      ...
      collapsedPreview={
        <p className="text-xs text-gray-400">
          {quantized} quantized Linear layer(s) ({inv.int8} INT8, {inv.e4m3} FP8
          E4M3, {inv.plain} unquantized)
        </p>
      }
