...
from core.training.arch.sensenova_sdxl_chimera import SenseNovaSDXLChimeraArchHandler
...
ARCH_REGISTRY: Dict[str, Type[ArchHandler]] = {
    ...
    "sensenova_sdxl_chimera": SenseNovaSDXLChimeraArchHandler,
...
def resolve_arch_name(trainer: Any) -> str:
    ...
    if getattr(trainer, "is_acestep", False):
        return "acestep"
    ...
    if getattr(trainer, "is_sensenova", False):
        return "sensenova"
    if getattr(trainer, "is_sensenova_sdxl_chimera", False):
