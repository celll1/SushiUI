...
| `code` | string (optional) | stable identifier for the notice, for clients matching on something other than text. Absent when the emitter supplied none. Current codes: `sensenova_stochastic_rounding_forced`, `sensenova_save_format_degenerate`, `sensenova_save_format_not_resumable`, `sensenova_save_format_resume_needs_base`, `sensenova_resume_base_accepted`, `sensenova_resume_state_incomplete`, `fp8_base_dtype_ignored`, `fused_gradient_accumulation_ignored`, `fused_grad_clipping_ignored`, `fused_grad_clip_ignored`, `grad_spike` (level `info` — broadcast, never persisted, because it is the one notice that repeats), `interrupted_checkpoint_save`, `component_lr_mismatch`, `component_lr_zero`, `component_lr_flattened`, `component_lr_resume_unavailable`, `training_event_cap_reached` |
| `resources` | array (optional) | Local files relevant to the notice. Each item has `kind`, `label`, and `path`. Grad-spike notices use this for the batch image and derived caption-sidecar paths; clients should offer both a local-file link and a copyable path. |
