...
| Activation offload / dispatch | Training only, opt-in for LoRA/ReLoRA/full parameter | `BaseTrainer` resolves settings from the shared run config; the video workload key includes 5-D clip length `T` and has an independent predictor. `LayerOffloadConductor` is built with `enable_activation_offload=False` in `ltx2_ops.setup_block_swap`. |
