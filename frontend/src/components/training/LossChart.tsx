...
const calculateRobustYRange = (
  ...
  return {
    min: Math.max(0, pMin - padding), // Don't go below 0 for loss values
    max: pMax + padding
  };
};
...
export default function LossChart({ runId, isRunning }: LossChartProps) {
  ...
  // Calculate min/max considering both loss and recon_loss using percentile-based range
  // This excludes outliers for more readable charts
  const allValues = [
    ...(showLoss ? lossData.map((d) => d.value) : []),
    ...(showReconLoss ? reconLossData.map((d) => d.value) : [])
  ];
  const { min: autoMinLoss, max: autoMaxLoss } = calculateRobustYRange(allValues);
