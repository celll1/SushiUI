...
| `frontend/src/components/generation/VideoInpaintRangeTimeline.tsx` | The `/generate/inpaint/video` range control: draws every VAE latent-group boundary from `video_constraints.latent_chunk_pattern` and snaps both handles to one, so a built request needs no server-side snap. |
