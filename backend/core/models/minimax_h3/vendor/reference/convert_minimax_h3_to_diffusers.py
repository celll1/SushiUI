# REFERENCE MATERIAL ONLY — NOT IMPORTED, NOT EXECUTED BY SushiUI.
#
# Source: https://github.com/huggingface/diffusers, branch `minimax-h3`,
#         file `scripts/convert_minimax_h3_to_diffusers.py`, retrieved 2026-08-05.
# Upstream file license: Apache-2.0.
#
# Kept verbatim as the Rosetta stone for the original-checkpoint -> diffusers
# key mapping (`get_transformer_key_plan` / `convert_transformer_key` /
# `convert_video_vae_key`). It imports `diffusers.models.autoencoders.
# autoencoder_kl_minimax_h3_audio`, which does not exist in the installed
# diffusers 0.38.0, so it is deliberately placed in a directory with no
# `__init__.py` and is never imported. This directory is not a Python package.
#
# NOTE (measured, SushiUI 2026-08-05): this script consumes the *original*
# MiniMax reference shards, whose fused qkv rows are per-head interleaved and
# are normalized by `reorder_interleaved_qkv` before `convert_transformer_key`.
# The Comfy-Org single-file checkpoints are already `[q_all; k_all; v_all]`,
# so only `split_fused_qkv` applies to them. See
# `scratchpad/minimax_h3_phase0_findings.md`.
