...
| 旧 `backend/core/training/aesthetic_loss.py`（2026-09-10 削除） | 未接続の frozen scorer 試作は forward が `torch.no_grad()` 内で入力勾配も切れていた。微分可能な補助損失の前例ではない |
...
- 旧 `backend/core/training/aesthetic_loss.py` — 未接続かつ入力勾配のない試作のため 2026-09-10 に削除
