...
| sdxl | U-Net | CLIP ViT-L/14 (768) + OpenCLIP ViT-bigG (`CLIPTextModelWithProjection`, 1280, penultimate + pooled) | epsilon / ddpm; v-pred via modelspec/`v_pred` | latent, `AutoencoderKL` 4ch, sf 0.13025 | same as sd15 (`abs(cfg-1)>1e-5`); caption dropout trains the shared U-Net's text-null response while size/crop micro-conditioning remains | conduit; tq/flash/sage usable (head_dim 64) infer; tq/flash/native train (`attention_impl=conduit`) | `.safetensors` (>6GB heuristic or `XL` class), diffusers dir | `sdxl_adapter.py`; dual-TE LRs; time_ids micro-cond; see `CFG_UNCONDITIONAL_TRAINING.md` |
...
| anima | DiT (Cosmos-Predict2 style) | Qwen3-0.6B (`Qwen3Model`) + 6-layer LLM Adapter; T5 tokenizer feeds adapter target ids | velocity / flow (rectified flow) | latent, Qwen-Image VAE (`AutoencoderKLQwenImage`-style, 16ch) | (verify) standard CFG via `_anima_encode_nag_neg` | model-local conduit for infer/train, including the LLM adapter; tq reaches unmasked calls and masked calls resolve native | split-files layout (`split_files/diffusion_models` \| `text_encoders` \| `vae`) or single DiT `.safetensors`; Qwen3 + Qwen-Image VAE auto-discovered by filename patterns | `anima_adapter.py`; DiT + optional LLM-Adapter-only training; Qwen3 TE + VAE frozen |
...
| qwen_image_21 | block-causal single-stream DiT, 32 blocks, hidden 4096 | Qwen3-VL, hidden 4096 (frozen; text plus native image context) | velocity / flow, `t0`, target `noise - x0` | RGBA latent, `AutoencoderKLQwenImage21`, 64ch, 16x, per-channel mean/std | true CFG only when scale >1 and a negative prompt exists; default cfg 1.0, 40 steps | model-owned exact segmented attention: native SDPA by default; training also accepts `flash`, which packs each sample/segment into FlashAttention varlen calls (causal text uses bottom-right alignment; image blocks are bidirectional). `tq`/`sage` are refused for training. Style Transfer remains native because it extends segment K/V | source Diffusers directory or variant manifest with separate single DiT/TE/VAE safetensors; Original and INT8 ConvRot variants; optional T2I/I2I Qwen3.5-VL 9B prompt enhancers stored separately as INT8 ConvRot single files | `qwen_image_21_adapter.py`; generation supports attention-projection LoRA strength/ranges, native reference images, latent Reference Guide, and segmented-attention Style Transfer; compatible ControlNet does not exist and is refused; training supports LoRA on Original or frozen ConvRot (128 real targets) and complete-DiT full fine-tune on Original only; TE/VAE frozen; ReLoRA/ControlNet/reference-edit training refused; prompt upsampling can use the bundled PE or a loopback LM Studio/Ollama model and never changes canvas geometry |
...
| sensenova | Qwen3-8B LLM used directly as the flow-matching denoiser (MoT: every layer carries a `_mot_gen` twin of q/k/v/o_proj + norms + MLP, selected per token by a boolean mask; 42 layers, hidden 4096, GQA 32 q / 8 kv heads, head_dim 128, 3-axis RoPE) | none separate — the prompt goes through the LLM's own tokenizer + chat template, encoded by the same prefix forward pass that builds the KV cache the denoiser consumes | flow matching, `linspace(0,1)` with **t=0 noise, t=1 clean** (opposite of flux2/zimage's convention), Euler forward, `v = (x_pred - z)/(1-t).clamp_min(t_eps)` | pixel-space (no VAE); resolution free (not bucketed), only the structural /32 token grid (patch 16 x merge 2) enforced by snapping, off-range sizes warn and generate | ordinary CFG via two independent prefix KV caches (cond + empty-string uncond by default), or up to three (cond + img_cond + uncond) once `ref_images` is supplied, selected by `img_cfg_scale`; upstream's own `needs_cfg = cfg_scale > 1` collapses to single-branch at `cfg_scale <= 1`, which is how the 8-step distillation LoRA runs — no separate mode flag. **`negative_prompt` IS supported** (empirically verified A/B, not just upstream's own default): the uncond branch is built by the SAME `encode_prompt()` prefix pass as the cond branch, so a caller-supplied string substitutes cleanly for `""` and produces a real, `cfg_scale`-dose-dependent suppression effect (clean up to ~cfg_scale 6, classic CFG-too-high degradation from ~cfg_scale 8); default `cfg_scale` 4.0 is kept for both the plain and negative-prompt cases. Has no effect at `cfg_scale<=1` (warned, `sensenova_negative_prompt_no_cfg`) since no uncond branch is built there at all | conduit-routed (BSHD); sage auto-refused for GQA declaratively (`supports_gqa=False`), no per-arch registry entry / same conduit, stamped `AttentionMode.TRAINING` over all 42 layers by `training/ops/sensenova_ops.setup_attention_backend` (sage stripped to native by the shared training-mode guard). **Understanding branch (`forward_und`)**: eager by default; reaches the conduit only under `train_text_encoder=True` AND `causal_fastpath` (mask provably degenerates to plain causal — text-only prefix, no reference images; `is_plain_causal_thw_index`). Reference-conditioned prefixes always stay eager (classifier returns False), which is also the long-sequence case where the shipped default (`attention_backend="native"`) **ISOLATED ATTENTION MICRO-BENCHMARK (attention-op-only harness, not a training-step or whole-run measurement)** measured ~21% faster / ~34% less peak memory with flash at L=1600 vs L≈350-450 text-only (~4%) — so the fast path is disabled exactly where it would help most, and only reachable on the minority `train_text_encoder=True` runs. `dispatch.py`'s conduit auto-sets `enable_gqa=True` for unequal q/kv heads on the native backend, and the same isolated harness measured PyTorch SDPA's `enable_gqa=True` path ~9x slower than pre-expanding K/V at this geometry (32q/8kv, head_dim 128); `causal_fastpath` pre-expands K/V only when the resolved backend is native (flash broadcasts GQA natively, so expanding there would only cost memory). The same `enable_gqa` cost applies to any other conduit-routed architecture with unequal q/kv heads resolving to native — Krea2's image-branch attention (48 q / 12 kv heads, head_dim 64) passes `enable_gqa=self.num_heads != self.num_kv_heads` explicitly and defaults to `attention_backend="native"`, so it hits the identical slow path by default; not fixed here (out of scope). **Production-scale measurement (real int8→bf16 checkpoint, FullParameterTrainer, `train_text_encoder=True`, both eviction flags on, adafactor, dataset 37, batch_size 1, gradient_checkpointing, 256x256, 3 warmup + 10 measured steps, RTX 6000 Ada) found the whole-step effect correctness-neutral and performance-neutral: fast_native −0.65% / fast_flash +1.81% vs eager, both inside run-to-run noise (~1.1-1.2% stdev), because the und prefix is only 7.5-8.3% of step time even though it improves 6.9-7.9% in isolation — the full table, the K/V-divergence figures and the resolution-scaling argument were condensed out of SENSENOVA_TRAINING_DESIGN.md by `86291fa1` and are not tracked anywhere; the figures here are what survives (256px is the MOST favourable resolution for this change and still shows no whole-step signal)** | sushiUI single-file shard index only (`transformer.`-prefixed, `sensenova_config` metadata carries geometry); no upstream single-file distribution to complete siblings against | `sensenova_adapter.py`, **LoRA and full fine-tune** — `relora`/`controlnet` are the refused pair (`TRAINING_UNSUPPORTED`; note ControlNet's only other guard, `controlnet_trainer.py`'s `is_zimage or is_flux2` check, runs AFTER the model loads, so `train_runner._apply_sensenova_training_contract`'s method allow-list is the free refusal). **LoRA**: the 294 generation-branch (`_mot_gen`) Linears by default, **plus 294 understanding-branch ones when `train_text_encoder` is set** (Phase U-1, `3d837202`..`327276df`; both vision towers stay frozen either way). **Full fine-tune (U-2-2, accepted 2026-08-25)**: the default generation scope is **gen-all** — 294 dequantized generation Linears, all generation-only decoder RMSNorms, and `fm_modules` (gen ViT, timestep/noise embedders and `fm_head`) in the generation optimizer component at `unet_lr`. `sensenova_train_fm_modules` defaults on and can exclude only the flow container; explicit task scopes separate `generation_decoder`, `generation_norms`, and `generation_flow`, so `generation_decoder` alone preserves the former Linear-only behavior. The understanding half remains separately selectable and has completed real measured runs. The full-FT contract is **not negotiable and is checked before the 17.6 GiB load** (`assert_full_finetune_contract` + the runner's own copy): optimizer **`adafactor`, `adamw8bit_ringbuffer` or `lion8bit_ringbuffer`**, the latter two only with `optimizer_state_host_resident` (8-bit state as pinned host memory: 30.19 / 15.09 GiB over both MoT halves, against 32.9 / 16.5 GB of GPU state without it, which does not fit) — refused before the load either way; bf16 `weight_dtype`/`training_dtype` (forced), `gradient_accumulation_steps=1` (fused backward frees each gradient as it applies it, so nothing survives to accumulate), `num_optimizer_groups=0`, no `use_ema`, no grad scaler; `optimizer_stochastic_rounding` is **forced on** for this route when unset and announced on the `training_log` channel; an explicit `False` is refused before the load instead of being silently overridden. Output format is `sensenova_full_finetune_save_format` (`mixed` default / `bf16` / `int8`) — **only `int8` is a legal base for a NEW run**, but a run RESUMING its own checkpoint takes `mixed` (single-half) and `bf16` (both halves) losslessly through `sensenova_ops.accept_resume_shaped_base`, which decides on a class census of the constructed tree and requires the metadata to agree. A real resume has been run for `gen`/`mixed`; the `both`/`bf16` acceptance is the same write/read pair feeding the same check, which is an inference rather than a measurement. Base for either method: an int8 checkpoint with all 588 decoder Linears in ONE flavour (no bf16 base exists in this repo); full FT additionally requires plain `Int8Linear`, not ConvRot. Physical `batch_size=1` **unless `enable_bucketing` is on** (a batch is one pixel tensor at one resolution and one noise scale; above 1 the prompts are packed end to end along the sequence axis rather than padded, `71672449`). Training accepts `blocks_to_swap=0..41`; non-zero uses branch-aware `(MoT half, physical layer)` transfer keys, requires gradient checkpointing and a ring of at least two slots, leaves LoRA adapters resident, and writes full-FT updates back through the common mutable engine. MoT phase/four-phase eviction can additionally cover resident layers. This composition is implemented but awaits the deferred GPU matrix. Training-time sampling and `debug_latents` both work (`dc91bef1`). **Reference-conditioned datasets work (Phase 3, `7a09af52`..`611a4a24`)** — references are understanding-tower ViT tokens spliced into the prompt prefix, NOT VAE latents like FLUX.2's, and `use_reference_images` mixes ref/non-ref items per item in one run. The base is converted UNMERGED from the 8-step distillation LoRA specifically to keep the trainable lineage canonical |
...
| yue2 | 2-stage 3B Mixture-of-Transformers: the same 28-layer, hidden-2048 decoder carries AR text/semantic and NAR acoustic halves | embedded 151,643-token ordinary-text tokenizer; the AR model consumes style, structured lyrics, and optional/generated ABC directly | AR categorical sampling produces 25 Hz semantic codec tokens; NAR uses the fixed 32-step midpoint flow-matching solve with whole-song seeded FP32 noise | 64-channel acoustic latent at 25 Hz; Oobleck VAE decodes in FP32 to 48 kHz stereo, with full or exact-boundary tiled decode | semantic CFG uses a protocol-defined negative prefix, not caller negative text; default 1.0 for `full`/`melody`, 1.01 for `off` | model-local PyTorch SDPA; no shared attention selector; native SDPA training with real per-layer gradient checkpointing | one complete Comfy-packaged `.safetensors`: embedded tokenizer + ConvRot INT8 AR/NAR + FP16 VAE. Strict header-first preflight reads markers and the tokenizer but not full model tensors; no directory, sibling completion, external tokenizer, or VAE override | `yue2_adapter.py`; Phase-A `abc_ar` ordinary LoRA only over the frozen ConvRot INT8 AR half; 112 attention targets plus optional 84 MLP targets; style + structured lyrics + sibling `.abc` required; full/ReLoRA/ControlNet, semantic/NAR training and audio-VAE training refused; adapter metadata scopes generation to the ABC stage |

> **Training-image alpha contract (2026-09-22):** Qwen-Image 2.1 alone receives
> source RGBA pixels at VAE encode: PNG/WebP/palette transparency survives crop
> and resize, while RGB sources acquire an opaque alpha channel. Its latent-cache
> VAE namespace carries `source-rgba-v1`, so earlier white-composited latents
> cannot be reused. The other image training paths retain white-composited RGB:
> SD1.5/SDXL/Chimera, Z-Image, Flux2/Ideogram4/Lens, Anima/Krea2, MiniT2I,
> and SenseNova have RGB pixel encoders or RGB pixel-space targets in their
> current model contracts; a four-channel *latent* does not imply RGBA pixels.
> Video still-image targets (LTX-2.3/MiniMax-H3) are RGB as well. REPA's
> SigLIP2 teacher remains RGB, independently of Qwen's RGBA VAE target.

> **Qwen-Image 2.1 training measurement (2026-09-21, RTX 6000 Ada):** real
> 256x256, batch-1 runs used the local artifacts and dataset 38. Rank-4 LoRA on
> the frozen INT8 ConvRot base completed losses 0.330088, 0.293226, then resumed
> from step 2 and completed step 3 at 0.316250; each adapter was 16,826,096
> bytes. Original full-DiT BF16 training used gradient checkpointing, 24/32
> swapped blocks, ring size 2, and host-resident stochastic-rounding
> `adamw8bit_ringbuffer`. Step 1 loss was 0.330214 and resumed step 2 loss was
> 0.373368. The 7,115,124,736-parameter DiT saved as four shards containing
> 14,230,249,472 tensor bytes; optimizer state was 14,453,079,941 bytes. The
> production loader reloaded the step-1 index with the frozen TE/VAE supplied by
> its terminal companion. A sampled whole-device peak was about 30.2 GiB but
> included the already loaded generation backend, so it is not a trainer-only
> VRAM figure. Keeping one complete model+optimizer bundle needs roughly 26.7
> GiB free plus filesystem headroom; the resumed run deliberately exercised the
> low-space path and exposed Windows temporary-file locking and chained-
> companion bugs, now covered by regression tests.
>
> A production-shape rank-128 LoRA follow-up used the INT8 ConvRot base,
> gradient checkpointing, batch 1 and mixed 1024/1280/1536 buckets. Direct CUDA
> events on eight native calls measured median forward+backward 10.708 s,
> backward 8.100 s (75.5%). The exact packed FlashAttention path measured
> 3.293 s and 2.273 s (69.2%) on its next eight calls. The sample-resolution
> order was not identical, so the median ratio is not a controlled speedup;
...
is the clean-end value. This applies to SD1.5/SDXL, Z-Image, non-distilled
FLUX.2, Ideogram 4, Lens, MiniT2I, Anima, non-distilled Krea 2, SenseNova U1.5,
and SenseNova SDXL Chimera, including their training-preview samplers where available.
Distilled guidance vectors and the audio/video-specific guidance protocols are not
relabelled as classifier-free guidance schedules.

> **Attention capability correction (2026-09-12):** the SenseNova row's older
> `supports_gqa=False` statement is superseded by the installed SageAttention
> API and the conduit registry. Sage accepts its 32-query/8-KV grouping for
...
- **Matched shared-model null training:** SD1.5, SDXL, Z-Image, Anima, and the
  non-distilled FLUX.2 / Krea2 routes use compatible empty-text representations
  in training and CFG inference. SDXL retains size/crop `time_ids`; reference,
...
- **Special cases:** LTX-2.3 aligns the empty-text branch but current training
  supervises video only, not the joint audio null field. Ideogram 4 trains its
  separate unconditional transformer on every sample. MiniMax Music 3 has AR
  logit CFG and flow CFG at inference but no training entry in this repo.
...
- **Arch-agnostic core**: `backend/core/inference/reference_style.py` —
  ...
  `frequency_scale_vector`, `StyleContext`. Adapts the MIT-licensed community
  `ComfyUI-Krea2-StyleTransfer` reference node's math; per-arch
...
- **ideogram4** — Only architecture bundling two transformers (conditional +
  ...
  attention. VAE is `AutoencoderKLFlux2` at 32 latent channels. The mathematical
  audit of its separate-branch training and the distinction from caption dropout is
  in `docs/guides/CFG_UNCONDITIONAL_TRAINING.md`.
  ...
  - **Offline artifact / export**: `EXPORT_LAYOUTS["ideogram4"]` is the only
    ...
    Measured on `<MODEL_ROOT>/ideogram4/ideogram4`: 669 source keys -> 805 canonical
...
- **sensenova** — SenseNova-U1.5-8B-MoT, vendored under `models/sensenova/vendor/`
  ...
  - **A second, opt-in ConvRot int8 checkpoint exists and IS covered by a
    passed measurement gate** (`<MODEL_ROOT>/sensenova/sensenova_int8_convrot.safetensors`,
  ...
  - **8-step distillation LoRA**: applied over the already-int8-quantized base
    ...
    - **Application now defaults to BOTH branches** (`e811e461`), so a
      gen+und LoRA trained by Phase U loads whole. Before that, `apply_lora_group`
      enumerated generation targets only, and an understanding entry in the file
      was **never looked up — the applied count just came out lower, with no
  ...
  - **Phase 1 LoRA training is implemented**; `"sensenova"` is one of
    ...
    - **Targets**: 294 generation-branch Linears
      (42 layers x {q,k,v,o}`_proj_mot_gen` + `mlp_mot_gen.{gate,up,down}_proj`),
      plus 294 understanding-branch ones (the same layout with plain names:
      `self_attn.{q,k,v,o}_proj` + `mlp.{gate,up,down}_proj`) when
      `train_text_encoder` is set. Both are enumerated by the SAME
      `iter_sensenova_lora_targets(branch=...)` the inference LoRA uses — one
      ...
      - **5 of the 294 und targets are structurally unreachable** and that is
        ...
        three MLP projections receive no gradient, while its `k_proj`/`v_proj`
        do (generation layer 41 consumes their K/V). Enumeration keeps all 294
        for symmetry; `und_gradient_unreachable_paths()` names the five so a
        census expects **289 of 294 by name** instead of failing on them.
        Measured on the real checkpoint.
      - Combining `train_text_encoder` with `sensenova_mot_phase_eviction` is
        **refused by the training contract**, not silently disabled — both are
        opt-in, so dropping either breaks a contract the user set. The message
        says it is a scope limit of this implementation, not a fundamental
        incompatibility.
    - **Two-pass step**: the prompt prefix is built by a `no_grad`
      `forward_und` pass (**differentiable, under autocast, when
      `train_text_encoder` is set**) and consumed as an immutable KV cache by
      the gradient-carrying `forward_gen` pass, which reads it through the
      `update_cache=False` concat path. `ops/sensenova_ops.py` asserts
      structure (layer count, non-empty K/V, no inference flash buffers, no KV
      streamer) unconditionally, then asserts **detachment when the und branch
      is frozen and the OPPOSITE — every layer's K/V carrying a `grad_fn` —
      when it is trainable**; a relaxed-but-not-inverted assert would let a
      prefix accidentally built under `no_grad` train nothing while the loss
      fell normally. A trainable prefix builds its cache from **checkpoint
      outputs** via an opt-in vendor `return_kv` seam, because a cache write is
      a side effect a non-reentrant recompute would duplicate. Gradient
      checkpointing runs through a SenseNova-specific per-layer
      non-reentrant loop, not the stock `GradientCheckpointingLayer`, which
      would replace `past_key_values` with `None` and silently drop the
      prefix conditioning.
    - **Accepted base** (`_assert_supported_quantized_training_base`,
      `0c9ea86b`): all 588 decoder Linears must be ONE quantized flavour, and
      that flavour must be `Int8Linear` or `ConvRotInt8Linear` — i.e. either
      shipped int8 checkpoint, plain or ConvRot. The census keys on
      `type(m) is cls`, never `isinstance`, because `ConvRotInt8Linear`
      subclasses `Int8Linear` and an isinstance census would fold a ConvRot
      base into the plain-int8 count and accept a MIXED base silently.
      `Fp8Linear`/`W4A8Linear` are censused for the diagnostic message but
      NOT accepted (no such SenseNova base exists, so accepting one would
      ship an untested path), an unknown subclass of a known quantized class
      refuses by name, and a mixed, off-count or unquantized bf16 base
      refuses too.
      - **ConvRot carries a train/inference activation-quantization skew;
        the plain-int8 checkpoint does not.**
        `ConvRotInt8Linear.forward` branches on
        `self._force_dequant or (torch.is_grad_enabled() and x.requires_grad)`,
        so once a LoRA contribution enters the activations essentially every
        Linear takes the differentiable dequant arm (W-int8 / A-bf16) during
        training, while INFERENCE always runs the fused W8A8 kernel —
        `disable_int8_mm` is inert on this class, whose `forward` never reads
        the flag. A ConvRot LoRA is therefore fitted against a base that
        differs from the deployed one by the activation-quantization error.
        **This skew is unmeasured**: nobody has A/B'd a ConvRot-trained LoRA
        under the fused inference kernel, so it is recorded as a known
        constraint, claimed neither harmful nor harmless. Autograd itself is
        consistent — the fused arm only runs where no gradient is required,
        and the non-reentrant checkpoint recompute evaluates the same
        predicate as the first pass. The plain-int8 checkpoint has no such
    ...
    - **Loss**: pixel-space flow matching in the arch's own `t=0` noise /
      ...
      than reimplemented.
    - **Refused before the model loads**: `full_finetune`, `relora` and
      `controlnet` (`arch_capabilities._add_training_unsupported`), so a
      client filters its method dropdown from the same source. Full
      fine-tuning has no bf16 base to run on — every one of the 588 decoder
      Linears holds its weight as an `Int8Linear` BUFFER, so
      `requires_grad_(True)` is a no-op on it (the Anima/Ideogram 4 guard
      class).
    - **Preflight contract** (`train_runner._apply_sensenova_training_contract`):
      `network.type` must be `lora`; `batch_size` must be 1 (effective batch
      via gradient accumulation — the prefix lengths of two items do not
      ...
      `use_reference_images` is type-normalized but **no longer refused**
      (`7a09af52`); `train_text_encoder` is accepted but **refused in
      combination with `sensenova_mot_phase_eviction`** (`e811e461`); text and
      ...
      A non-zero `attention_dropout` is refused once und training is armed
      (`assert_understanding_training_supported`): the vendor applies dropout
      in `train()` mode, so a checkpointed prefix **recompute** would not
      reproduce the K/V of its own forward. Upstream's default is 0.0, so this
    ...
    - **Reference-conditioned training (Phase 3, `7a09af52`..`611a4a24`)**:
      `use_reference_images` now admits SenseNova, and per-item
      `reference_images` may be mixed with reference-free items in one run.
      The mechanism is **not** FLUX.2's: a reference is opened inside
      `ops/sensenova_ops._load_reference_images` and handed to the vendor's
      `load_image_native` untouched (per-reference smart-resize + ImageNet
      stats through the understanding tower), then spliced into the prompt
      prefix as `<IMG_CONTEXT>` tokens. It never reaches `encode_image`.
      - **Two of the six `use_reference_images` gates stay FLUX.2-only on
        purpose** (`base_trainer.py`, the two `use_reference_images and
        self.is_flux2` branches): those are the VAE-latent path, and
        SenseNova is released from them by having its own entry, not by being
        added to them. A test pins that occurrence count at exactly 2
        (`sensenova_reference_training_test.py`), so "widening" them to
        include SenseNova is a test failure, not a fix.
      - `train_step`, the immutability assert, the checkpoint loop and the
        phase evictor are unchanged — a longer prefix is all they observe.
      - `SenseNovaTrainingPrefix.text_length` became `indexes[0].max()+1`
  ...
  - **`sensenova_mot_phase_eviction`** (API boolean, default **off**,
    ...
    - **A separate TRAINING implementation of the same concept exists**
      ...
      feature's layer-selection logic (persistence, not `nn.Parameter`
      ownership) but not its inference callbacks: a
      `full`/`prefix`/`denoise` state machine CPU-stages the generation
      half for the `no_grad` prefix pass and the understanding half for the
      LoRA forward/backward, always evicting before loading at a transition
      so the two halves never co-reside, and holding the generation half
      GPU-resident across forward/backward/optimizer/save. A partial
      transfer failure poisons the evictor rather than being retried. The
      training preflight refuses it together with
      `num_optimizer_groups > 0` or `block_swap_h2d_only`. Its measured
      OFF/ON numbers, and what that measurement does and does not settle,
      are summarized in `SENSENOVA_TRAINING_DESIGN.md` rather than duplicated
      here.
    - **`sensenova_mot_pageable_staging`** (API boolean, default **off**,
      ...
      returns a pinned block to the OS; see `SENSENOVA_TRAINING_DESIGN.md`);
  ...
  - **`sensenova_kv_cache_streaming`** (API boolean, default **off**,
    ...
    - **No applicability to `train_step`**: a training step is a
      ...
      MoT half-eviction CONCEPT, and it has since been built: Phase 1 LoRA
...
- **yue2** — YuE2 text-to-music generation is exposed only through
  `POST /generate/txt2aud` and the existing audio mode in `Txt2ImgPanel.tsx`.
  - **Prompt contract**: `prompt` is the music/style description and `lyrics`
    must be non-empty structured lyrics. `yue2_cot` selects `full`, `melody`,
    or `off`; the first two can generate ABC or consume caller-supplied
    `yue2_abc`, while `off` rejects supplied ABC. There is no score-only API.
  - **Owned staged lifecycle**: AR ABC/semantic generation, chunked NAR
    acoustic solving, and VAE decode are sequential residency phases. The NAR
    solver is fixed to 32 midpoint steps and preserves the whole-song seeded
  ...
  - **Artifacts**: the FLAC's ordinary generation metadata is accompanied by
    ...
    truncation flags, timing, hashes, and model identity. They do not enable
    audio extend/repaint.
  - **Provenance**: the vendored implementation is adapted under Apache-2.0
    from `multimodal-art-projection/YuE` commit
    `88da114a67df892af0329472073b96a5ef700b93`; weights are CC BY-NC 4.0.
    See `docs/legal/THIRD_PARTY_PROVENANCE.md`.
...
- `backend/core/models/yue2/{loader,pipeline,artifacts}.py` — complete-file
