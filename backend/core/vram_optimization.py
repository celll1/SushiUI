...
def move_deus_unet_to_gpu(unet, quantization: Optional[str] = None, use_torch_compile: bool = False):
    """Move DEUS U-Net to GPU for inference, optionally with quantization and torch.compile

    Args:
        unet: DEUS U-Net model
        quantization: Quantization type - None, 'none', 'fp8_e4m3fn', 'fp8_e5m2', etc.
        use_torch_compile: Whether to apply torch.compile for speedup

    Returns:
        unet: U-Net on GPU (may be quantized and compiled)
    """
    # Normalize quantization parameter
    if quantization in [None, "", "none"]:
        quantization = None
    ...
    if not quantization:
        print("[VRAM] Moving DEUS U-Net to GPU for inference...")
        print(f"[VRAM] torch.compile requested: {use_torch_compile} (type: {type(use_torch_compile)})")
        print(f"[VRAM] U-Net has _compiled attribute: {hasattr(unet, '_compiled')}")
        if hasattr(unet, '_compiled'):
            print(f"[VRAM] U-Net _compiled value: {unet._compiled}")
        
        unet.to('cuda:0', non_blocking=False)
        
        # Apply torch.compile if requested
        if use_torch_compile:
            if hasattr(unet, '_compiled') and unet._compiled:
                print(f"[torch.compile] DEUS U-Net already compiled, skipping compilation")
            else:
                print(f"[torch.compile] Compiling DEUS U-Net for optimized inference...")
                print(f"[torch.compile] Note: First inference will be slower due to compilation")
                print(f"[torch.compile] Subsequent inferences will be significantly faster (1.3-2x speedup)")
                try:
                    unet = torch.compile(
                        unet,
                        mode="reduce-overhead",  # Good balance for inference
                        fullgraph=False,  # Allow graph breaks for compatibility
                    )
                    unet._compiled = True
                    print(f"[torch.compile] Successfully compiled DEUS U-Net")
                except Exception as e:
                    print(f"[torch.compile] Warning: Compilation failed: {e}")
                    import traceback
                    traceback.print_exc()
            ...
            print(f"[VRAM] torch.compile not requested, skipping compilation")
        
        return unet

    # Quantization path (for future support)
    print(f"[VRAM] Moving DEUS U-Net to GPU with {quantization} quantization...")
    print(f"[VRAM] Note: Quantization for DEUS U-Net is experimental")
    
    # TODO: Implement quantization for DEUS U-Net if needed
    print(f"[VRAM] Warning: Quantization not yet implemented for DEUS U-Net")
    unet.to('cuda:0', non_blocking=False)
    return unet


def move_deus_unet_to_cpu(unet):
    """Move DEUS U-Net to CPU to free VRAM

    Args:
        unet: DEUS U-Net model
    """
    print("[VRAM] Moving DEUS U-Net to CPU to free VRAM...")
    if unet is not None:
        unet.to('cpu', non_blocking=False)
