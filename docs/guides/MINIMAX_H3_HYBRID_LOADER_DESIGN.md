...
### 4.6.1 実測結果（DiT のみ、2026-08-14）

実チェックポイントに対する `_build_transformer` の DiT 単体ロード。TE は意図的に含めていない（48 GiB マッピングを除外した測定）。1 アームずつ独立プロセスで実行。ホスト 93.6 GB、実行前の空き約 70 GB。

| arm | 形式 | ファイル各 | peak RSS | peak VMS | 同時マッピング数 | 秒 |
|---|---|---|---|---|---|---|
| base-only | w4a8_mixed | 11.68 GB | **10.47 GB** | 20.83 GB | 3173 | 13.8 |
| hybrid 25..49 | w4a8_mixed | 11.68 GB | **10.47 GB** | 32.54 GB | 3174 | 10.1 |
| base-only | fp8_scaled | 19.52 GB | **16.75 GB** | 55.83 GB | 3162 | 17.0 |
| hybrid 25..49 | fp8_scaled | 19.52 GB | **16.74 GB** | 55.83 GB | 3163 | 11.3 |
