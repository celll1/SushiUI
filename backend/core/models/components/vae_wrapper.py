"""
SDXL VAE Wrapper (component layer — plan A.4).

Canonical home for ``SDXLVAEWrapper`` / ``get_sdxl_vae``. The old
``core.models.sdxl_vae_wrapper`` module re-exports these (identity preserved).
Moved byte-identically (BEHAVIOR FREEZE); any pre-existing quirks are preserved
and logged to the follow-up list, not fixed here (plan R7/F).

SDXL VAE uses 4-channel latents (standard for SDXL). This wrapper uses the
FP16-fixed version from madebyollin/sdxl-vae-fp16-fix to avoid NaN issues when
...
class SDXLVAEWrapper(nn.Module):
    ...
    def to(self, device=None, dtype=None, **kwargs):
        ...
        # Handle device and dtype separately
