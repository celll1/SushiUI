...
#         `src/sensenova_u1/models/neo_unify/modeling_qwen3.py`, retrieved 2026-08-21.
# Upstream license: Apache-2.0 (see `backend/core/models/sensenova/vendor/__init__.py`).
#
# SushiUI modifications:
#   * `_flash_or_sdpa` (the sole attention call in the generation-branch
#     forward, `Qwen3Attention.forward_gen`) now routes through SushiUI's
#     unified attention conduit (`core.attention.dispatch_attention`) instead
#     of its own flash-attn/SDPA branch, so the repo-wide `attention_type`
#     vocabulary (native/flash/sage/tq) selects the kernel here as on every
#     other architecture. The backend/mode are read off the calling
#     `Qwen3Attention` instance's `_attn_backend` / `_attn_mode` attributes
#     (default `"native"` / `AttentionMode.INFERENCE`), set by whichever unit
#     drives generation — the same shape as MiniMax-H3's
#     `_attn_backend` / `_attn_mode` stamping (see
#     `backend/core/models/minimax_h3/vendor/transformer_minimax_h3.py`).
#     q/k/v are already `[B, S, H, D]` (BSHD) at the call site (upstream's own
#     "Flash layout" comment), which is the conduit's canonical layout, so no
#     transpose is needed. The module-level `_ATTN_BACKEND` selector
#     (`set_attn_backend`/`get_attn_backend`/`effective_attn_backend`) is
#     UNCHANGED and still exported, but `_flash_or_sdpa` no longer consults it
#     — kept for API compatibility with anything importing this module
#     standalone (e.g. the upstream `t2i_generate` reference path).
...
def _flash_or_sdpa(q, k, v, dropout_p: float = 0.0, softmax_scale=None, causal: bool = False,
                   attn_module: "Qwen3Attention | None" = None):
    """SushiUI: routes through the unified attention conduit.

    ``q``/``k``/``v`` are ``[B, S, H, D]`` (BSHD) at the sole call site
    (``Qwen3Attention.forward_gen``), which is the conduit's canonical layout.
    ``attn_module`` (the calling ``Qwen3Attention`` instance) carries the
    resolved ``_attn_backend`` / ``_attn_mode``, defaulting to native/inference
    when unset -- see the module header for why the old flash/SDPA branch was
    replaced rather than kept as a fallback.
    """
    backend = getattr(attn_module, "_attn_backend", "native")
    mode = getattr(attn_module, "_attn_mode", AttentionMode.INFERENCE)
...
class Qwen3Attention(nn.Module):
    ...
    # SushiUI addition: conduit backend/mode, read by ``_flash_or_sdpa``.
    # Class-level defaults (native/inference); whichever unit drives
    # generation sets these per instance, the same shape as MiniMax-H3's
