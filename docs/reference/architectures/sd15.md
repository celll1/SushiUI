...
* Training block swap: `SD15ArchHandler.setup_block_swap` remains a documented
  no-op. Fused optimizer groups are consequently unreachable too
  (`num_optimizer_groups` is only read under `blocks_to_swap > 0`). Generation
  has a separate forward-only U-Net stage conductor.
...
| Block swap boundary | `DiffusionPipelineManager._maybe_install_sd_unet_block_offload` → `FrozenModuleOffloadConductor` over `down_blocks`, `mid_block`, `up_blocks` | Generation supported for txt2img/img2img/inpaint and delegated outpaint; mutually exclusive with `torch.compile`. Training remains unsupported (`SD15ArchHandler.setup_block_swap` returns `None`). |
| Component offload | `core.vram_optimization.move_text_encoders_to_gpu/cpu`, `move_unet_to_gpu/cpu`, `move_vae_to_gpu/cpu`; state printed by `log_device_status` | Supported and composed with generation block swap; a split U-Net is not marked keep-hot. |
...
| Training block swap and fused optimizer groups are unavailable | `SD15ArchHandler.setup_block_swap` (no-op) + the training capability declarations for `block_swap` and `num_optimizer_groups` |
