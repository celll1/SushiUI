...
| Path | Responsibility |
|---|---|
| `frontend/src/components/generation/Txt2ImgPanel.tsx` / `Img2ImgPanel.tsx` / `InpaintPanel.tsx` | The three generation panels: params state, UI controls, loop-generation step params, FormData/apiParams construction. **There is no separate video or audio panel**: when `StartupContext` reports the loaded model as `isVideo`/`isAudio`, these panels switch mode (Txt2Img → txt2vid/txt2aud/ref2vid, Img2Img → img2vid) and render the video/audio controls and `<video>` output inline. |
| `frontend/src/components/common/MiniMaxH3ReferenceSelector.tsx` | The omni-reference uploads of a `/generate/ref2vid` request (images, videos with an optional positional soundtrack, standalone audio), rendered by `Txt2ImgPanel.tsx` only when the loaded MiniMax-H3 partition is `ref2va`. The upload order is the packed order, so the control keeps it. |
