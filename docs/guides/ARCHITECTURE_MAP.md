...
| `backend/core/models/<arch>/` | Per-architecture component loaders and vendored model classes (e.g. `minimax_h3/loader.py` + `h3_pipeline_ops.py` + `vendor/`, `ltx2/loader.py`, `acestep/`). A vendored architecture owns its denoise loop here when upstream ships no usable `DiffusionPipeline`. |
...
| `backend/api/routes.py` | All FastAPI endpoints. The image generation endpoints (`/generate/txt2img`, `/generate/img2img`, `/generate/inpaint`) are `multipart/form-data` (`Form(...)` params), not JSON; the video/audio ones (`/generate/txt2vid`, `/generate/txt2aud`) take a JSON body, and the ones that carry a file (`/generate/img2vid`, `/generate/outpaint/video`, `/generate/outpaint/audio`) are multipart. `_reject_if_video_model` / `_reject_if_audio_model` keep each family's endpoints from accepting another family's loaded model. |
...
| Path | Responsibility |
|---|---|
| `frontend/src/components/generation/Txt2ImgPanel.tsx` / `Img2ImgPanel.tsx` / `InpaintPanel.tsx` | The three generation panels: params state, UI controls, loop-generation step params, FormData/apiParams construction. **There is no separate video or audio panel**: when `StartupContext` reports the loaded model as `isVideo`/`isAudio`, these panels switch mode (Txt2Img → txt2vid/txt2aud, Img2Img → img2vid) and render the video/audio controls and `<video>` output inline. |
