...
def train_and_save(tmp_path, name="zimage.safetensors"):
    ...
    return str(out), model, wrapped_paths(model)
...
def test_zimage_wrapped_forward_is_base_plus_scaled_branch(tmp_path):
    ...
    for target in sorted(trained_paths):
        ...
        down = saved[f"{stem}.lora_down.weight"]
        up = saved[f"{stem}.lora_up.weight"]
...
def test_zimage_alpha_beats_the_rank_fallback(tmp_path):
    ...
    modules = dict(model.named_modules())
    scales = {round(modules[t].scale, 9) for t in trained_paths}
...
def test_zimage_fully_shadowed_second_lora_refuses_and_warns(tmp_path, warnings_seen,
                                                             resolve_verbatim):
    path, _trained, _paths = train_and_save(tmp_path)
    second, _t2, _p2 = train_and_save(tmp_path, name="second.safetensors")

    with pytest.raises(RuntimeError, match="already wrapped"):
        _Backend(build_model())._load_lora_zimage(
