...
#
# Vendored verbatim from `pid/_src/networks/flow_matching.py` in
# github.com/nv-tlabs/PiD (no framework dependencies). Only `FlowMatchingTrainer.timescale`
# is consumed by the vendored inference path (`_student_sample_loop`); the rest
# (training loss / sampler helpers) is kept for fidelity but unused at inference.
