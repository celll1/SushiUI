...
| GQA handling | `core/attention/registry.py` (Flash/Sage/TQ accept divisible GQA) and `dispatch.py`'s native pre-expansion | Unequal q/kv head counts; Sage remains inference-only |
| Block swap boundary | Generation: `SenseNovaMixin._sensenova_maybe_install_block_offload` → `FrozenBranchedLayerOffloadConductor`. Training: `SenseNovaArchHandler.setup_block_swap` → `BranchedLayerOffloadConductor` | Both key units by `(understanding|generation, physical_layer_index)`. Generation is immutable with a fixed two-slot ring and is mutually exclusive with MoT phase eviction; training uses method-aware writeback and requires checkpointing/ring >=2 |
...
| Activation offload / dispatch | Training only, opt-in for LoRA/full parameter | Flow batches use the image workload family. Instruction/text batches use mean input-token rows and a separate predictor, preserving their token-weighted micro-batch reduction. Physical `batch_size=1` runs can offload but cannot split further. |
...
| REPA tap (training-only) | `SenseNovaArchHandler.repa_tap` -> `language_model.model._repa_tap_depth` / `._repa_tap_out`, written by `sensenova_ops.forward_gen_decoder_layers` and read by `sensenova_ops.train_step` | The generation stream only: that loop runs the gen half against the understanding half's prefix K/V, so the tap carries no text rows and needs no slice. Cleared at every forward; unarmed the loop neither reads nor writes it. Both sequence shapes are accepted -- `[B, token_h*token_w, D]` and the packed `[1, B*token_h*token_w, D]` -- and regrouped as the `fm_head` line below the loop does, rows row-major `h*token_w + w`. Grid is `(token_h, token_w)`, i.e. one row per `patch * vae_scale_factor` px, not per latent cell. Under four-phase with `train_text_encoder`, the alignment gradient reaches the prefix boundary leaves like the diffusion one does, so it updates the understanding half too |
