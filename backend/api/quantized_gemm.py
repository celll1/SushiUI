    ...
    * the label resolved to a ``dequant`` stem (``dequant...`` for FP8,
    ...
    try:
        ...
        if not label:
            ...
            part.startswith(("w4a8_int8", "convrot_int8"))
            for part in label.split("+")
        ):
            message = (
                "quantized_gemm_mode='w8a8' does not control this checkpoint's fixed "
                f"quantized operator (resolved path: {label}); its Comfy-Kitchen path runs "
