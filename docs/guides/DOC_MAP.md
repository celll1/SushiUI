...
| `docs/guides/SENSENOVA_TRAINING_DESIGN.md` | Design record and DONE/PENDING boundary for SenseNova U1.5 training: Phase 1 gen-branch-only LoRA over the int8 base is **implemented** (arch handler + ops + adapter + `ARCH_REGISTRY`, two-pass prefix/denoise step, opt-in MoT half-eviction with its OFF/ON measurement in §8.3); the full fine-tune is still refused end to end (`arch_capabilities` + `train_runner`), though three pieces have landed — `cc296e84`'s load-time route that dequantizes the MoT half a full FT would train back into real `nn.Parameter`s, and `601d0271`'s adapter that collects it (`SenseNovaFullParameterAdapter`), its `assert_full_finetune_contract`, and the decoupling of the fused backward pass from Block Swap; and `24220b5c`'s U-2-3 (stochastic rounding forced on for this route rather than defaulted, because the transport cannot express "unset", plus the dropout guard made unconditional for full FT); all are proven by tests rather than by a run (§6.4, §13.4 U-2-2, §6.3, §13.3). What remains is the refusal's removal, and **two preconditions are still open for it: the output checkpoint format decision (§6.4 keeps three candidates open deliberately) and a user-visible notice channel for the forced override, which today reaches the backend server's console only** — read §13.4's warning box before opening anything. Reference-included datasets mixed with plain ones are **Phase 3** (done). Records why the understanding branch is frozen by default (its LoRA is opt-in via `train_text_encoder`, Phase U-1), why the recorded block-swap refusal does not transfer from generation to training, and what the repo does and does not actually know about quantized-base and bf16-rounding constraints. |
