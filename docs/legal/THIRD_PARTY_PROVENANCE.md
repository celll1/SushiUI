...
| `backend/core/models/qwen_image_21/vendor/` | Hugging Face Diffusers PR 14804, commit `6256aa7` | Apache-2.0 | Qwen-Image 2.1 model terms; weights remain external under the user's model directory | **verified at package level**; `vendor/UPSTREAM.md` records the pinned revision and integration changes |
...
| `backend/core/models/yue2/vendor/` | `https://github.com/multimodal-art-projection/YuE`, commit `88da114a67df892af0329472073b96a5ef700b93` | Apache-2.0 for the adapted YuE2 model, protocol, and sampling code; the package retains `vendor/LICENSE` and per-file modification markers | CC BY-NC 4.0 for the YuE2 weights. SushiUI loads the Comfy-Org packaging as one complete safetensors file containing the tokenizer, ConvRot INT8 AR/NAR tensors, and FP16 VAE; the repository does not redistribute that file | **verified at package level** for the pinned YuE source and adaptation status. `modeling_vae.py` records stable-audio-tools/SnakeBeta MIT ancestry in `THIRD_PARTY_NOTICES.md`; both full MIT texts are bundled under `vendor/licenses/` |
