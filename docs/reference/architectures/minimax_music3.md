...
| Block swap boundary | supported independently in both compute phases | `_minimax_music3_maybe_install_block_offload` → `FrozenModuleOffloadConductor`; AR uses `language_model.model.layers`, flow uses `transformer.transformer_blocks`; teardown occurs at each phase boundary |
...
| Activation offload / dispatch | **not applicable** — this architecture has no training handler or objective; the generation pipeline has no activation dispatcher | absence from `core.training.arch.ARCH_REGISTRY`; `core.pipeline_backends.minimax_music3` |
