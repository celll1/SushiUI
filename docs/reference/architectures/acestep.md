...
| Activation offload / dispatch | Training only, opt-in | Generic `BaseTrainer` dispatch uses ACE-Step's 3-D `[B, T, 64]` latent as an audio workload keyed by `T`; its predictor is independent from image/video/text families. `LayerOffloadConductor.enable_activation_offload=False` avoids stacking the older conductor mechanism. |
