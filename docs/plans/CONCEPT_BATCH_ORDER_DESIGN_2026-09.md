...
3. **state 保存**: `concept_batch_plan` に version、実効 seed、設定 hash、分類入力 hash、crop plan fingerprint、当該エポックの最終 batch 数、最終 batch 列の digest を保存する。再開時は設定 seed が `-1` または保存値と異なっても保存した実効 seed を再使用し、notice を出す。digest は各 batch の順序付き `(dataset ID, image_path, bucket key, concept ID, base/replay)` から作り、crop spec と参照画像条件も含める。計画本体は保存しない。既存のエポック内 `batch_idx` は最終 batch 列に対する次バッチ位置のまま使う。
