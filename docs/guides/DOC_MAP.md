...
| `docs/guides/SENSENOVA_TRAINING_DESIGN.md` | Design record and DONE/PENDING boundary for SenseNova U1.5 training: Phase 1 gen-branch-only LoRA over the int8 base is **implemented** (arch handler + ops + adapter + `ARCH_REGISTRY`, two-pass prefix/denoise step, opt-in MoT half-eviction with its OFF/ON measurement in §8.3); the full fine-tune is still refused end to end (`arch_capabilities` + `train_runner`), though its first piece landed in `cc296e84` — the load-time route that dequantizes the MoT half a full FT would train back into real `nn.Parameter`s, proven by tests rather than by a run (§6.4); the rest is **deferred to Phase 2b / U-2-2**. Reference-included datasets mixed with plain ones are **Phase 3** (done). Records why the understanding branch is frozen by default (its LoRA is opt-in via `train_text_encoder`, Phase U-1), why the recorded block-swap refusal does not transfer from generation to training, and what the repo does and does not actually know about quantized-base and bf16-rounding constraints. |
