...
`cfg_uncond_drop_rate=q_max`, it requires stochastic loss selection and uses a
hierarchical three-way draw. First, guided loss is selected with probability
`p(sigma)`. Only otherwise-selected ordinary-MSE items are eligible for a null
caption, with conditional probability `q_max`. The resulting marginal
probabilities are guided `p(sigma)`, null MSE
`(1-p(sigma))*q_max`, and conditional MSE
`(1-p(sigma))*(1-q_max)`. A null-labeled image receives ordinary flow MSE under
the inference empty-prompt condition and is ineligible for guided loss.
...
One categorical draw fixes both the loss choice and null mask per MNT
iteration, before OOM recovery, and is reused by every retry and micro-batch
slice. MNT>1 requires
