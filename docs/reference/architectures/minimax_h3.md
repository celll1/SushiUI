...
| Activation offload / dispatch | Training only, opt-in for LoRA | `BaseTrainer` resolves dispatch settings from the shared run config; the video workload key includes the 5-D clip length and uses an independent predictor family. `LayerOffloadConductor` is built with `enable_activation_offload=False`. The measured H3 boundary is recorded in `docs/audits/MINIMAX_H3_ACTIVATION_DISPATCH_GPU_RESULT_2026-09.md`. |
