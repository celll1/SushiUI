...
SenseNova branch conductors and the hook-driven generation routes were added
after the measurements above. They share the validated transport primitives,
but the synthetic results do not establish architecture-level numerical parity,
throughput, or VRAM savings for those routes. Such claims require fresh,
architecture-specific measurements; proposed campaigns remain machine-local
until executed.
