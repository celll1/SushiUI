...
def sdxl_write_dora(tmp_path, source_name, name="dora.safetensors"):
...
def sdxl_recording_pipeline(monkeypatch, *models):
...
def test_sdxl_a_dora_file_refuses_before_diffusers_ever_sees_it(
    ...
    _directory, name, *_paths = train_and_save(tmp_path)
    dora = sdxl_write_dora(tmp_path, name)

    models = (build_unet(), *build_text_encoders())
    pipeline, reached = sdxl_recording_pipeline(monkeypatch, *models)
    with pytest.raises(RuntimeError) as excinfo:
...
def test_sdxl_an_ordinary_lora_is_byte_identical_with_the_gate_neutralised(
    ...
    with_gate = (build_unet(), *build_text_encoders())
    pipeline, reached = sdxl_recording_pipeline(monkeypatch, *with_gate)
    ...
    without_gate = (build_unet(), *build_text_encoders())
