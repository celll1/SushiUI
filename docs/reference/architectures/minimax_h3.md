...
| Activation offload / dispatch | Training only, opt-in for LoRA | `BaseTrainer` resolves dispatch settings from the shared run config; the video workload key includes the 5-D clip length and uses an independent predictor family. `LayerOffloadConductor` is built with `enable_activation_offload=False`. Real-checkpoint validation remains in `docs/audits/ACTIVATION_DISPATCH_GPU_VALIDATION_BACKLOG_2026-09.md`. |
