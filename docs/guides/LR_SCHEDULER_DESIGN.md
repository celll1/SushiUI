...
| D45 | トリガが見る信号 | トレーナーが毎 step 既に計算している量に限る: `loss`、`grad_norm`、`extra_metrics` の任意のキー。**`learning_rate` は対象外**（自分が変える量を条件にするのはフィードバックループで、意味論が定まらない）。値は DB から読まない: メトリクス算出地点で**メモリ上のリングに積み**、トリガは §5.2 seam (c) の poll でそれを読む。毎 step の DB 読み出しを学習ループに入れない |
| D46 | 述語の語彙 | `plateau`（`patience` 回の観測で `min_delta` を超える改善が無い）、`below`（閾値未満）、`above`（閾値超）の 3 つ。**閾値・`min_delta`・`patience`・観測窓はすべて必須項目で、既定値を持たない**。良い `min_delta` はモデルとデータセットの loss スケールに完全に依存し、既定を与えることは §16-12 が禁じる「未測定の数値」を書くことに等しい。形だけを提供し、数値は運用者が持つ |
