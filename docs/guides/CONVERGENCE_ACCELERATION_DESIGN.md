...
| 資産 | 実装から確認できること |
|---|---|
| `backend/core/training/base_trainer.py::_setup_repa` と `repa.py` | 教師は tagger / SigLIP2 選択式。**当初 MiniT2I 専用だったのは実装のゲートであり手法の制約ではない**（2026-09-08 に横断化）。`ArchHandler.repa_tap()` を実装したアーキで有効。配線済み: MiniT2I, Anima, Lens, Krea 2, Ideogram 4, SenseNova U1.5 |
...
REPA の正方形スカッシュは形状を変えるが、品質低下の大きさは未測定。
latent と教師画像の crop/flip/resize の対応を確認した上で、前処理だけを変える対照実験が必要。
...
| G-B | スカッシュ検証は教師前処理以外を固定し、非正方形バケツで空間対応と生成品質を比較。教師が異なる系の REPA loss の大小だけでは判定しない |
