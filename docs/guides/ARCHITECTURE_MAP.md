...
│   │   ├── adapters/        # LoRA/LoHa/LoKr/DoRA engine (arch-neutral) + execution/
...
| `backend/core/pipeline_backends/` | One file per architecture (`zimage.py`, `flux2.py`, `anima.py`, `lens.py`, `krea2.py`, `ideogram4.py`, `minit2i.py`, `sensenova.py`, `sensenova_sdxl_chimera.py`, plus the non-image `ltx2.py`, `minimax_h3.py`, `acestep.py`, `minimax_music3.py`, `yue2.py`; SD1.5/SDXL are handled by the base `pipeline.py` path) — architecture-specific generation logic as mixins. |
| `backend/core/models/sensenova_sdxl_chimera/` | Chimera artifact builder/loader, understanding-only source loader, fixed-length conditioning bridge, parameter-free three-axis-RoPE attention processor, and flow sampler. The artifact bundles an SDXL-shaped U-Net and VAE while hash-pinning the external SenseNova understanding source. |
