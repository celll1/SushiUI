...
| REPA tap (training-only) | `SDXLArchHandler.repa_tap` -> `sd_sdxl_ops.repa_tap` (one body with `sd15`); a `register_forward_hook` armed on the tapped block by `sd_sdxl_ops.train_step` and removed in its `finally`, writing `unet._repa_tap_out`; read by `repa.apply_repa_loss_spatial` | Supported. A conv feature map `[B, C, h, w]`, not a token sequence: the teacher is interpolated to that map's own `(h, w)` and flattened row-major (`h*gw + w`), which makes the projector's channel-axis Linear a 1x1 convolution. `repa_align_depth` indexes three sites — `down_blocks[2]`, `mid_block` (auto default), `up_blocks[0]` — because a U-Net's blocks have no total order by depth; out of range is refused, not clamped. Width read live from `resnets[-1].conv2.out_channels`, which for all three is `block_out_channels[-1]`. The hook is outside every checkpoint segment (diffusers checkpoints each resnet INSIDE a block, `use_reentrant=False`), and a detached tap is refused rather than contributing zero. A ControlNet run is refused: `train_step_controlnet` never reads the tap and the U-Net is frozen there. REPA was published for DiTs (arXiv:2410.06940); a conv U-Net mid block is an extrapolation from it. |
