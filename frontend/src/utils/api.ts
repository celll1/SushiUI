...
export interface ModelInfo {
  source_type: string;
  source: string;
  type: "sd15" | "sdxl" | "zimage" | "flux2" | "anima";  // DEUS support removed
...
export const exportSigLIP2ONNX = async (output_path: string, max_num_patches: number = 256, strip_unknown_tags: boolean = false) => {
  const response = await api.post("/tagger/siglip2/export-onnx", { output_path, max_num_patches, strip_unknown_tags });
