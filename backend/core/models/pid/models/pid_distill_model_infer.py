...
#
# Vendored (trimmed) from `pid/_src/models/pid_distill_model_infer.py` in
# github.com/nv-tlabs/PiD.
#
# Dropped relative to upstream: the `tokenizer`/`vae_encoder` branch and
# `encode_lq_latent()` — SushiUI's SDXL config always sets `tokenizer=None` (we
# already have the raw SDXL latent; PiD's `from_ldm` path never touches a VAE), so
# `self.vae_encoder` was always `None` in our usage and the whole branch (plus its
# `lazy_config.instantiate` import) is dead code here.
#
...
from core.models.pid.models.pixeldit_model import PixelDiTModel, PixelDiTModelConfig
...
class PidInferenceModel(PixelDiTModel):
    ...
    def _resolve_inference_image_size(self, lq_latent: Tensor, image_size=None) -> tuple[int, int]:
        """Resolve output H/W from an override or from latent/sr scale (no VAE)."""
        if image_size is not None:
            ...
            # Upstream also supports inferring the output size from a configured VAE's
            # spatial_compression_factor; SushiUI's SDXL config never sets a VAE
            # (tokenizer=None), so image_size must always be passed explicitly here.
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        Returns:
            [B, 3, 1, H, W] tensor in [-1, 1] (the trailing temporal dim of 1 exists
            for callback compatibility with upstream's video-capable callbacks;
            callers decoding a single SDXL latent should `.squeeze(2)`).
        """
